...
import evaluation.constants as e_constants
from evaluation.evalDatasets import EvaluationHvoDataset

from tests.constants import *

import numpy as np

HVO_DATASET = EvaluationHvoDataset(EVALUATION_SET_PATH)
...
def test_get_features_dict():
    extracted_features = featExt.get_features_dict(HVO_DATASET, features_to_extract=e_constants.EVAL_FEATURES)

    assert len(extracted_features) == len(e_constants.EVAL_FEATURES)
    for feat in e_constants.EVAL_FEATURES:
